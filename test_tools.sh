...
if [ "$FILTER" = "all" ] || [ "$FILTER" = "medium" ]; then
  ...
  test_tool "get_scan_status" '{"state": "Running,Paused,Queued,Error", "days": 7}'
  test_tool "get_scan_status" '{"state": "Error", "days": 1, "limit": 10}'
  ...
  test_tool "get_edr_events" '{"days": 7, "severity": "CRITICAL"}'
  test_tool "get_edr_events" '{"days": 30, "category": "ransomware"}'
  test_tool "get_edr_events" '{"days": 7, "host": "DESKTOP"}'
  test_tool "get_fim_events" '{"days": 1}'
  test_tool "get_fim_events" '{"days": 7, "severity": "CRITICAL"}'
  test_tool "get_fim_events" '{"days": 7, "path": "/etc/passwd"}'
  test_tool "get_fim_events" '{"days": 1, "host": "prod"}'
