...
func (m *Module) RegisterTools(s *server.MCPServer) {
	s.AddTool(
		mcp.NewTool("car_list_scripts",
			mcp.WithDescription("List custom assessment and remediation scripts. Scripts can be used to detect issues or remediate vulnerabilities on assets."),
	...
	s.AddTool(
		mcp.NewTool("car_get_script",
			mcp.WithDescription("Get details of a specific custom script including its description, platform, and configuration."),
	...
	s.AddTool(
		mcp.NewTool("car_execute_script",
			mcp.WithDescription("Execute a custom script on specified assets. Can run in test mode first to validate. Returns a correlation UUID to track job status."),
	...
	s.AddTool(
		mcp.NewTool("car_list_jobs",
			mcp.WithDescription("List script execution jobs. Shows status, success/failure counts, and timing."),
	...
	s.AddTool(
		mcp.NewTool("car_get_job_results",
			mcp.WithDescription("Get detailed results from a script execution job. Shows per-asset status, output, and exit codes."),
	...
	s.AddTool(
		mcp.NewTool("car_list_remediation_scripts",
			mcp.WithDescription("List available remediation scripts. These scripts fix vulnerabilities or misconfigurations on assets."),
			mcp.WithNumber("limit", mcp.Description("Maximum number of scripts to return (default 100)")),
		),
