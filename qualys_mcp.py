    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            if e.code == 500 and '/cdr-api/' in url:
                _log(f"CDR findings endpoint returned 500 — returning unavailable: {url.split('?')[0]}")
...
def _paginate_json(base_url, limit, data_key='data', count_key='count',
    ...
    while True:
        ...
        if data in ('KB_BUSY', 'CDR_UNAVAILABLE'):
            return data
        try:
...
def get_cloud_risk(limit: int = 20, include_threats: bool = True, days: int = 7) -> dict:
    ...
    if include_threats:
        ...
        by_provider = {}
        by_category = {}
        cdr_raw = eval_results.get('cdr')
        if cdr_raw == 'CDR_UNAVAILABLE':
            result['cdr_message'] = CDR_UNAVAILABLE_MSG
            findings = []
        else:
            findings = cdr_raw or []
...
def get_threats(days: int = 7, limit: int = 50) -> dict:
    ...
    cdr_raw = concurrent.get('cdr')
    if cdr_raw == 'CDR_UNAVAILABLE':
        result['cdr_message'] = CDR_UNAVAILABLE_MSG
        cdr_findings = []
    else:
        cdr_findings = cdr_raw or []
